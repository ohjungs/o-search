...
**마크업 모양 축도 여기 안 들어 있다 — 이 도구의 세 번째 눈멂이다** (계획 48 개발 8).
코퍼스는 동결된 **평문**이고 위 `wrap` 이 문장마다 `<p>` 하나로 감싸므로, 여기 나오는
정확도·채택률은 **한 문장짜리 `<p>` 위에서만** 잰 값이다(코퍼스에 `<br>` 0건 ·
`nav`/`ul`/`h*` 0건 · 블록 최소 길이 129자). 실제로 `<br>`·내비·목록·표가 문단 경계를
망가뜨려도 **이 두 숫자는 한 자리도 안 움직였다**(개발 8 실측: 수리 전후 398/398 ·
99.5% · p95 1.36ms 로 동일). 그 축을 재는 자는 `tests/test_indexer.py` 의
`_SHAPES` 표다 — 모양마다 «사람이 인용할 문단» 을 적어 두고 매 반복 돈다.
