...
| plan_digest-rotate | 진행 | loop/digest-rotate | (미개설) 1/2 | 없음(사용자 표면 0 · 제품 0줄) | **38.** 근거는 집안일 — `docs/digest.md` 가 **232줄·90,568바이트**로 머리말 상한 200 을 넘겼고(`status.md` 가 두 반복 연속 "밀린다" 고 적었다), discover 1~8순위에는 열 것이 없다(단위 456 OK · 코드 `TODO` 0건 · 후보는 전부 도달 불가·천장 수용·**사람이 정할 갈림길**). **탐색이 덤으로 손상을 찾았다** — `b11fd40`(2026-08-27) 이후 H1 `# 아카이브 요약` 이 리스트 항목 안으로 들어갔고, 그 사고로 018 이 닫은 `[5] 호스트 대소문자` 항목이 **4일째 열린 후보로 보인다**(`urls.normalize('http://A.com/') -> 'http://a.com/'` 로 닫힘 실측). **회전을 두 반복 막던 선행 조건("참조 확인")은 이미 무너져 있었다** — `digest.md:<줄번호>` 인용 8곳 중 **6곳이 빈 줄이나 다른 항목**을 가리킨다(인용 당시엔 맞았다, `git show 1abc058`). 2스텝: ① 머리 복구 + 닫힘 표시를 `digest.md:135` 제자리로 ② 취소선·`[원문]`·2026-08-25·26 완료 7줄 삭제 → **187줄 예상**. 불변량은 열린 항목 **47** 과 `## 반복 실패` **3항목**. 기점 `c2a115e` |
