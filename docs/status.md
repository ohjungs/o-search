...
note: **빈손 두 밤을 컨셉으로 끊었다.** 여덟 출처는 이번에도 전부 0건이지만(전수 788 OK · `TODO` 0 · 이슈 0 · `candidates.md` 없음 · 보류는 보안 건 하나) 계획 77 이 쓴 길을 다시 썼다 — `docs/specs/concept.md` 를 자로 댄다. **경량 2**(검색 서버 상주 메모리 2GB · 판정: RSS 측정)가 **판정 방법까지 적힌 채 한 번도 안 돌아간 축**이다(RSS 재는 코드 **0줄**). 계획 102 `serve-rss` 착수. 설계는 다음 반복 — 트리거 둘(새 파일 · 대안 2개 이상)
---
