...
## 2026-08-25 야간35 | search-api | 개발 5/5 e2e | 시도1
- 한 일: e2e/search_api_e2e.py 신설 — crawl 15페이지 수집·색인 후 `python3 -m websearch.serve <db> --port 0` 를 서브프로세스로 띄우고 stdout 첫 줄에서 실제 포트를 읽어 HTTP 로 검증. docs/e2e/search-api/result.md 기록, project.md e2e 명령 확정
- 결과: e2e 통과 15.0s(수집 간격 정책). 1페이지 10건 has_next 참 / 2페이지 5건 거짓 / 겹침 0 / 400·404·501 다섯 건 트레이스백 없음. 전체 115/115, 기존 e2e 3개 회귀 없음
- 판단: 문서 15개 — 2페이지가 나오되 딱 떨어지지 않게 해서 마지막 페이지를 5건으로 만들었다. 딱 떨어지는 경계(20문서)는 단위 테스트가 이미 본다. 시나리오에 없던 page=0·101 을 더했다: 검증이 CLI 기동 경로에서도 사는지가 단위 테스트로는 안 보인다
- 다음: 테스트 phase(갭 탐색) — 계획 스텝은 5/5 소진했지만 테스트·리뷰 관문을 아직 안 지났다

## 2026-08-25 야간36 | search-api | 테스트 | 시도1
- 한 일: rules/test.md 3절 갭 탐색 — 탐침으로 ① 없는 DB ② 색인 전 DB ③ 동시 요청 8건 ④ 픽스처의 bm25 동점 여부를 실측. 8점 3건 처리: 동시 요청 계약 테스트, 색인 전 DB 200 빈결과, 없는 DB 500 에 경로·트레이스백 없음
- 결과: 118/118 통과 0.53s. 변이 검사 3건 — 연결을 스레드 밖에서 재사용하면 동시성 테스트가 500!=200 으로 FAILED(check_same_thread=False 로는 아예 SIGSEGV), 색인 전 가드 제거 시 ERROR, 500 본문에 예외를 실으면 FAILED
- 판단: **페이지네이션 픽스처가 bm25 동점이었다** — 20건 전부 점수가 같아(서로 다른 값 1/20) 페이지 경계 단언이 검증하던 것은 관련도순이 아니라 sqlite 의 동점 처리 순서였다. 낱말 수를 문서마다 달리해 점수를 갈랐다. 초록불이던 테스트가 실제로는 계약을 안 잡고 있던 경우다
- 8점 미만 digest 로: [5] serve.main 인자 처리 무테스트
- 다음: 리뷰 phase (백지 패스는 별도 세션에 위임해 진행 중)

## 2026-08-25 야간37 | search-api | 리뷰 | 시도1 → DONE
- 한 일: 백지 패스를 별도 세션에 위임(diff·소스만, 계획/설계 문서 미제공) → 80점 이상 4건. 코드에서 재확인하고 재현 테스트 3건 먼저 작성·실패 확인 후 수정 — ① has_next 가 MAX_PAGE 를 몰라 page=100 에서 참, page=101 은 400 (has_next 를 따라가는 클라이언트는 반드시 400) ② 접근 로그를 끄려 log_message 를 덮어 log_error 까지 죽었다 → 500 이 stderr 에도 안 남음, log_request 만 덮도록 ③ 소켓 타임아웃 없음(슬로로리스로 스레드 무한 점유) → timeout=10 ④ bm25 동점에 2차 정렬 키 없음
- 결과: 124/124 통과 0.96s, e2e 4개 회귀 없음. 기준선 재측정 p95 6.71ms(3000문서)
- 판단: **④ 는 리뷰 제안(`, url`)을 안 쓰고 `, rowid` 로 갔다.** 리뷰가 "공짜"라고 했지만 실측하니 2만 문서에서 url 은 p50 13→27ms 두 배였고 rowid 는 12.8ms 로 값이 0 이었다. 이 계획이 연 축이 성능이라 제안을 그대로 받지 않고 재봤다. 리뷰 지적 4건 모두 실재, 오탐 0
