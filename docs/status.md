...
note: **e2e — 네 판 전부 돌렸다**(`docs/e2e/read-budget/result.md` · 깨끗한 worktree 에서). **① 자가 옳게 세나**: `wc -l` 과 자가 인쇄한 수가 네 파일 전부 일치(180·63·272·79 = **594/600**) — 손으로 두 번 틀린 수를 이번엔 **다른 자로** 대조했다. **② 사용자가 하는 그대로**: SKILL.md 규격인 **5줄** 기록을 붙여 가니 **밤 2 째에 물었다**(604) — **여유가 6줄이다**. 되돌리니 gap `None`·594 로 복귀. **③ 전수 안 3건 실패 중 진짜는 하나**: `ReadBudgetTest` 만 604>600 이고 `HistoryCapTest`·`IterationSyncTest` 는 **내 합성 픽스처 부산물**이다(항목 19+2=21>20 · 반복 번호 700·701). 덤으로 **항목이 오늘 19/20 이라 회전이 한두 밤 앞**이다. **④ 변이 9종 전부 죽는다** — M5·M6(갈래 하나뿐)과 M7·M8·M9(명부에 하한 없음)가 처음엔 살았고 못을 박아 다시 쟀다. **품질 기준은 `src/` 0줄이라 의존성 0 과 전수 809 만 돌았고 나머지 여섯 축은 「해당 없음」으로 적었다 — 통과라고 쓰지 않는다.** 남은 것은 마감(아카이브 → `merge-to-main.sh`)
---
