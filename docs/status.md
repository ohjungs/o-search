...
ctx: 측정 불가 — 게이트 ⑦ **열여섯 번째 재현**. `.context-state.json` 의 `session_id`(`c26c9e20…`)로 `~/.claude/projects/-Users-t2025-m0191-work-websearch/<id>.jsonl` 을 찾으니 **없다 — 남의 세션 파일이다**. 게다가 **2.9시간** 낡았고(`updated_unix` 1789369054 · 지금 1789379455) `five_hour_resets_at` 도 지났다. statusLine 꺼진 것과 동일 취급 · `SKILL.md` 3절대로 **반복 상한에만 의존**했다
rules: (저장소 밖 `~/.claude/skills/loop-harness` — 커밋 해시 없음)
note: 이 밤은 **빈손이다 — 계획 0건**. 여덟 출처가 전부 0건(전수 788 OK · 코드 `TODO` 0 · 이슈 0 · `candidates.md` 없음 · 보류는 보안 건 하나)이고, digest 후보에 남은 것은 **사람 판단(게이트 ⑫·⑥ 등)·이미 닫힌 것·저장소 밖** 셋뿐이다. 빈손으로 닫기 전에 **한 번도 안 잰 축**을 쟀다: **테스트 순서 의존 0건**(788건을 평평하게 펴 씨앗 1·2·3 으로 섞어 세 판 전부 788 OK). 코드 0줄 · digest `[5]`(`sys.path` 잠복)에 값으로 등재. 정지 사유 「탐색 막힘」
---

## 반복 593 — 빈손 밤 · 「순서 의존」 축을 처음 쟀다 · 벽시계 함정 4회째

**계획 0건이다.** 여덟 출처가 전부 0건이고(전수 **788 OK** · 코드 `TODO` 0 · 이슈 0 ·
`candidates.md` 없음 · 보류는 보안 건 하나), digest 후보에 남은 것은 셋뿐이다 —
**사람 판단**(게이트 ⑫·⑥ 등)·**이미 닫힌 것**(`hidden-passage`·`robots-nonascii`)·
**저장소 밖**(`~/.claude` 공유 도구). `loaf`←`loaves` `[6]` 은 열 수 있어 보이지만
**FTS5 의 `porter` 는 우리 코드가 아니다** — 여는 길이 질의 확장 + 불규칙 복수 사전이라
제품 랭킹 경로를 건드리는 설계 갈림이고, 합격선(en 80%)은 **19/20 으로 이미 넘어 있다**.

**빈손으로 닫기 전에 한 번도 안 잰 축을 쟀다 — 테스트 순서 의존.** 788건을 평평하게
펴서 씨앗 1·2·3 으로 섞어 돌리니 **세 판 모두 788 OK**(20.5·20.5·20.7초). 재기 전에
**섞임이 실제로 일어났는지 먼저 단언했다**(같은 자리 0~1/788) — 계획 101 이 배운
「대리 측정이 0을 세고 있지 않은지를 함께 단언한다」를 그대로 적용한 자리다.
**마감에서 벽시계 함정이 네 번째로 났다** — `merge-to-main.sh` 가 **600초 상한**에 걸렸는데
같은 로그 안에서 전수는 **22.879초**였고 병합·푸시가 `rc=0` 으로 끝나 있었다(`bc2cf78`).
명령은 성공했고 **셸만 멈춰 있었다**. 값은 digest `[5]`(`sys.path` 여섯 칸 **잠복**)에 등재했다 — 그 항목의 「지금은 가릴
이름이 0개」가 눈으로 훑은 값이었는데 **이제 실측이 받친다**. 코드 **0줄**.

## 계획 101 `e2e-fd-leak` — 마감(2026-09-14 야간 반복 588~591)

**4/4 완료 · e2e 통과 · `main` 병합 대기.** 계획서 `docs/plan_history_076.md` ·
설계 `docs/design_history_076.md` · 결과 `docs/e2e/e2e-fd-leak/result.md`.

- **근거가 어디서 나왔나가 이 밤의 요점이다.** 여덟 출처는 이번에도 전부 0건이었다
  (전수 784 OK · 코드 `TODO` 0 · 이슈 0 · `candidates.md` 없음 · 보류는 보안 건 하나).
  앞 밤은 여기서 「탐색 막힘」으로 닫았다. 이번에 다른 것은 **등재 항목을 다시 읽은 것**이다 —
  digest `[4]` 는 반복 586 이 「단위 쪽은 닫혔다」로 갱신해 뒀는데, **e2e 절반은 아무도
  안 쟀다.** 그 문장이 근거였다.
- **뚜껑이 둘이었다.** e2e 는 `-b` 를 안 쓰는데도 경고가 안 보인다 — 파이썬이
  `ResourceWarning` 을 **기본으로 무시**한다. 반복 586 이 연 것은 두 뚜껑 중 하나였다.
- **실측 30건 · 18종 중 16종 · 전부 rc=0 인 채로.** `tracemalloc` 으로 원인이 둘로 갈렸다:
  듣는 소켓 24(`shutdown()` 만 하고 `server_close()` 없음 · 17파일)와 자식 파이프
  6(`terminate()`+`wait()` 는 `communicate()` 와 달리 안 닫는다 · 5파일).
- **고친 뒤 22/22 · RW 0 · DW 0 · 단위 788 OK** · 품질 4축 회귀 없음 · `src/` **0줄**.
- **가드가 첫 판에 거짓 초록이었다** — `\bHTTPServer\(` 는 `ThreadingHTTPServer` 안에
  낱말 경계가 없어 **0개를 세면서** 17파일이 새는 위를 통과했다. 같이 넣은 「잴 것이 있나」
  단언이 잡았다. **대리 측정을 세울 때는 그 자가 0을 세고 있지 않은지를 함께 단언한다.**
- **남는 것**: `docs/project.md` 러너 명령에 `-W` 를 다는 일(게이트 ⑫)은 **그대로 사람 몫**이다.
  `tests/test_e2e_fd.py` 는 낱말을 세는 대리 측정이라 실측을 대신하지 못한다.
