...
| 의존성 0 | `-m unittest tests.test_deps` | 표준 라이브러리 밖 임포트 **0건** (경량 3·5) — 예산이 아니라 **불변식**이라 어기면 실패다. **표에서 유일하게 전수 안에서 도는 줄**이고, 그것이 이 자의 요점이다: 나머지 여섯은 사람이 기억해서 쳐야 하지만 이 축은 **안 쳐도 물린다** |
