...
note: **스텝 2 — 초록. 692 → 573**(`status.md` **299 → 180**). 접은 것은 **반복 서사 네 절**(594·595·598·601 · 121줄)이고, 이 파일이 매 반복 「덮어쓰기」인데 서사를 append 해 온 자리다(`status-read-budget` 이 일곱 밤치를 접은 것과 같은 진단). **네 절 다 원본이 따로 있다** — 594 → `history_087.md`(22줄) · 595·598·601 → `history_current.md`(18·15·11줄) · [R102-1~4] 는 `metrics.md` 리뷰 행. **대조는 두 겹으로 했다**: 토큰(백틱 식별자·`파일:줄`·해시·수) **유실 0건**, 줄 단위로는 **97 중 93 이 축자 중복이 아니다** — 접은 것은 같은 사실의 **더 긴 두 번째 서술**이지 유일본이 아니다. 이 구분을 숨기지 않는다. **유일본이라 안 접은 것 하나**: 「직전 계획 102 — `main` 에 들어갔다(`24edddd`)」 — 그 해시가 저장소에서 여기밖에 없다. **`규모 축` 의 손으로 적은 수는 지웠다** — 「375」가 낡았고 그것을 고쳐 적은 「525줄/600 · 여유 75줄」도 내역 셋이 전부 틀린 채 실측 603 을 가리고 있었다. 세 번째로 손으로 적는 대신 `ReadBudgetTest` 를 가리킨다. 전수 **808 OK**. **남은 것은 사람 몫**: `status.md` 180 은 제 상한 **50** 의 3.6배다 — 무엇을 남길지는 판단이라 밤이 안 연다
---
...
**손으로 적은 수는 두 번 조용히 틀렸다**: 「375」가 낡았고(반복 593 재측), 그것을 고쳐 적은
「525줄/600 · 여유 75줄」도 세 내역이 전부 틀린 채 **실측 603** 을 가리고 있었다(반복 609).
세 번째로 손으로 적는 대신 자를 세웠다. 넘기면 전수가 파일별 실측/상한을 인쇄한다.
