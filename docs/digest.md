...
- [8] **토크나이저가 못 잡는 세 가지가 검색 품질의 유일한 손실원이다** (2026-08-26 quality-eval 개발 4/4 실측). 미포함 5/40 이 전부 이것이고 랭킹 손실은 0건이다: 복합어 뒷부분(`김치찌개보관법` 을 `보관법` 으로 못 찾음) 2건 · 띄어쓰기 변형(`올레 길` ↔ `올레길`) 1건 · 영어 굴절(`loaves`←`loaf`, `tuple`←`tuples`) 2건. 형태소 분석기는 `concept.md:76` 이 "하지 않을 것" 으로 못박았으니 남는 후보는 **FTS5 `trigram` 토크나이저 병행 색인**(접미·부분 매치가 공짜로 붙는다, 색인 크기 증가가 대가)과 **영어 굴절만 다루는 질의측 s-접미 처리**(`tuples` → `tuples* OR tuple*`, 몇 줄). 둘 다 랭킹을 바꾸므로 이 계획이 만든 기준선(ko 85 / en 90)을 **먼저 고정한 뒤에** 손대야 비교가 된다
