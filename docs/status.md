...
note: **테스트 phase — 갭 탐색 ⑥ 이 거짓 초록 하나를 찾았다.** `read_budget_gap` 은 이빨이 있었지만 **명부 자체에 하한이 없었다** — `READ_BUDGET_FILES` 에서 `status.md` 한 줄을 빼는 변이(M7)에 합계에서 **180줄이 조용히 사라지고** 전수 112건이 그대로 OK 였다. **오늘 예산을 넘긴 바로 그 파일이 빠지는 변이고, 자가 제 입력을 줄여 초록이 되는 꼴이다**(계획 103 이 `test_deps.py` 에서 찾은 것과 같은 모양). 파일별 상한 50/60/300/80 도 같이 살았다(M8·M9) — 단언엔 안 쓰이지만 **실패 메시지가 사람에게 인쇄하는 수**라 틀리면 어느 파일을 볼지를 틀리게 알려준다. `ReadBudgetRosterTest` 로 셋을 한 번에 닫았다(룰 파일이 저장소 밖이라 임포트 대조가 불가 — 값을 적어 고정한다). **변이 9종 전부 죽는다.** **내 변이 스크립트가 먼저 나를 속였다**: 못을 박자 `assert count==1` 이 못 자체까지 세어 조용히 터졌고, 변이가 안 걸린 실행이 「OK」로 찍혀 **M7·M8 이 산 것처럼 보였다** — 앵커 실패를 화면에 내게 고치고 다시 쟀다. 맨몸 규율이 경고하는 「실패가 통과로 보인다」가 검사 도구 쪽에서 일어난 표본이다. 전수 **809 OK** · 예산 **574/600**
---
