...
### ② 정공법 — 새 모듈 (`src/websearch/flags.py`, 처음엔 `cli.py` 로 썼다)
...
## 2. 판정 — ② 새 모듈 (`src/websearch/flags.py`)
...
이 모듈은 함수 하나로 시작하고 **커지면 그때 나눈다.** 지금 넣지 않는 것:
문자열 플래그 헬퍼(`indexer --query` — 계획 밖), 불리언 플래그, usage 생성기.
추측성 확장은 기준 4번에서 감점이다.

**이름은 `cli.py` 가 아니라 `flags.py` 다** (리뷰 패스 A 가 잡았다, 반복 141).
README 5~15줄이 `python -m websearch.cli crawl ...` 를 안내하는데 **그 모듈은 없다**
— 지금은 `No module named websearch.cli` 로 rc 1 이다. 여기가 그 이름을 가져가면
`main` 도 `__main__` 블록도 없는 모듈이 **rc 0 으로 조용히 성공한다**(실측).
실패를 성공으로 위장하는 쪽이 더 나쁘다. README 자체는 이 계획 밖이라 안 고쳤다
(`digest.md` 후보로).
...
본다 — `--workers ≥ 1`·`--port ≤ 65535` 는 호출부마다 다르고(`--max` 는 0 도 받는다), 파서에
