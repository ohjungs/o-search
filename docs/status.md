...
note: **테스트 phase — 갭 둘을 찾았고 둘 다 8점 이상이라 닫았다.** 추측이 아니라 변이로 쟀다: ① 단언을 옛 자리(`REQUEST_LOG`)로 되돌리는 변이 **809 OK** ② 못을 `0.5` 로 내리는 변이 **809 OK**. **이 계획이 이틀 걸려 찾아낸 것이 전수에 아무 자국도 안 남긴다** — 9일을 조용히 산 두 줄이 다시 조용해질 수 있다는 뜻이라 8점으로 봤다. `tests/test_e2e_interval.py` 를 세웠다(`test_e2e_fd.py` 와 같은 관용구 — e2e **소스의 낱말**을 센다). 전수 **809 → 811**. **변이 넷 전부 죽는다**: 옛 자리 복귀 · deadline 못 0.5 · crawl_delay 못 0.5 · 단언 삭제(하한 못이 0건을 문다). **README 건수 가드가 또 강제했다**(809 → 811) — 계획 76·98 과 같은 자리다. **게이트 ⑨ 네 번째 피해**: autocommit 이 00:41 에 내 스텝 2 작업을 `b2b02f9` 「자동 스냅샷」으로 가져가 **origin 에 밀었다**. 내 커밋(`68bd6d5`)엔 `index.md` 한 줄만 남았다. 내용은 온전하고 브랜치도 `loop/*` 이라 피해는 이력이 갈린 것뿐이다 — **이미 푸시돼 되감지 않는다**(강제 푸시는 밤이 안 한다). **아침에 볼 것**: 스냅샷이 3분만 일렀다면 `DOMAIN_INTERVAL = 0.5` **제품 변이**를 커밋해 밀 뻔했다(실측: 이번 스냅샷엔 `src/` 0건). ㉯ 가드「트리가 더러우면 건너뛴다」가 그 문을 닫는다. 다음은 리뷰 phase.
---
