...
ctx: 측정 불가 — 게이트 ⑦ **스물두 번째 재현**. `.context-state.json` 은 **0분밖에 안 낡았는데**(신선도 검사 통과) `session_id`(`4b0e3020…`)에 해당하는 `.jsonl` 이 내 프로젝트 슬러그에 **없다 — 또 남의 세션이다**. 어젯밤과 **같은 session_id** 라 그 파일이 내 것이 아닌 채로 계속 갱신되고 있다는 뜻이고, `context_used 42` 는 내 값이 아니다. `five_hour 30`·`seven_day 55` 는 계정 단위라 참이고 둘 다 85 아래다 — **반복 상한에만 의존한다**(`SKILL.md` 3절)
rules: (저장소 밖 `~/.claude/skills/loop-harness` — 커밋 해시 없음)
note: **짧은 경로 1건 완료 — `interval-call-site`.** 어젯밤 계획 105 가 등재한 후보 `[6]`(여는 조건 없음)를 **하루 만에** 닫았다. **재는 자는 불려야 잰다** — `test_e2e_interval` 이 `check_intervals` 의 **몸**만 읽어서 호출 세 자리를 지운 트리가 `rc=0`·전수 **811 OK** 로 지나갔고, 오늘 그것을 **재현한 뒤** 호출 자리를 세는 단언을 세웠다. 변이 2판(전부 지움 3≠0 · 하나만 지움 3≠2) 둘 다 죽는다. **제품 `src/`·`e2e/` 는 바이트 단위로 무변경**이고 건수 가드가 **다섯 반복째** 물어 README 811→**812**. 후보가 함께 주장한 「같은 모양이 `test_e2e_fd`·`test_readme` 에도 있다」는 **실측으로 반증**했다 — 그 둘은 애초에 호출/사용처를 센다(아래 새 후보 참조). **정지 사유는 「탐색 막힘」이다 — 계획 상한(3)도 한도(5h 30%·7d 55%)도 아니다.** 반복 624 에서 여덟 출처를 전수로 다시 쟀고 **남은 것이 전부 ⓐ교훈 ⓑ이미 닫힌 것 ⓒ사람 판단 대기 셋 중 하나**였다 — 상세는 아래 「반복 624」 절. 없는 일을 지어내지 않았다(`discover.md` 2절).
---

## 짧은 경로 — `interval-call-site` (반복 623 · 완료)

- **근거**: `digest.md` 6순위 `[6]`(2026-09-18 계획 105 e2e 음성 대조 B) — 소스를 읽는
  가드가 「함수의 몸」만 보고 「불리는지」는 안 본다
- **할 일**: `tests/test_e2e_interval.py` 에 `check_intervals` **호출 자리**를 세는 단언
- **완료 기준**: 호출을 지운 변이에서 RED · 원상복구하면 GREEN · 전수 초록
- **이미 한 것**: **전부 끝났다** — 커밋 `38a0a8e` · 전수 **812 OK** · 변이 2판 통과


## 반복 624 — 탐색 전수 재측 · **빈손이고 그것이 결론이다**

여덟 출처를 값싼 것부터 직접 쳤다. **1~3 은 0건**(전수 812 OK · `src/`·`e2e/`·`scripts/`
의 `TODO`/`FIXME`/`HACK` **0건**). 4순위 큐(`docs/candidates.md`)는 **파일이 비어 있다** —
후보 `[6]` 이 등재한 「구조적으로 늘 0건」 그대로다. 5·6순위에서 열린 항목을 전부 훑고
**하나씩 근거를 실측해** 갈랐다:

- **ⓐ 교훈이라 계획이 안 되는 것** — 「거래를 재려면 양쪽을 같은 자로 재라」 「탐침의
  배선을 먼저 의심한다」 「변이가 실제로 심어졌는지를 먼저 단언한다」 등 `[8]`·`[7]`
  다수. 점수는 높지만 **할 일이 아니라 태도**다.
- **ⓑ 이미 닫힌 것** — `[7]` 비ASCII robots(계획 100) · `[7]` 숨은 텍스트가 근거 문단으로
  (계획 51 · README 96~99줄이 그 결과를 적고 있다) · `[7]` 낡은 바이트코드
  (`merge-to-main.sh` 가 `PYTHONPYCACHEPREFIX` 를 쓴다).
- **ⓒ 사람 판단 대기** — `[7]` 게이트 ⑫ `-W always`(고칠 자리가 `project.md` 의 자기 규율) ·
  `[6]` 4순위 큐(digest 84개 항목의 **표기 계약**) · `[5]` `status.md` 산문 드리프트
  (「산문을 재는 자를 들일 것인가」 — 이 저장소가 **두 번 일부러 피한 방향**이고, 함께
  적힌 값싼 대안은 오늘 `IterationSyncTest` 와 겹친다) · `[8]` `perf_crawl` 기준선(대기 ②).

**컨셉 축도 댔다**(계획 102·103 이 쓴 길). `concept.md` 의 수 있는 축 중 자가 없는 것을
찾았으나 **기능 6**(화면·JSON 순서 일치)은 `test_serve.py:547` 이 이미 물고, **성능 1·4**
는 `design_check` 의 LCP 대리(`SERVER_BUDGET_MS = 300`)와 `perf_search` 가 나눠 덮는다.
남은 **성능 3**(수집→검색 10분)은 오늘 파이프라인이 **사람이 손으로 거는 배치**라
「지연」의 정의부터 사람 몫이다 — 자를 세우면 재는 게 아니라 **정의를 지어내는 것**이다.
