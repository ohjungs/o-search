...
note: **e2e phase 통과 — 계획 105 의 기대 결과 셋이 다 섰다.** 기록은 `docs/e2e/crawl-gap-ab/result.md`(78줄). 앞 관문: 전수 **811 OK**(21.1s 맨몸) · 린트·타입체크는 이 저장소에 없다 · **CI 설정 파일이 없어 CI 관문은 해당 없음**. ①`deadline_e2e` **5판 연속 rc=0**, 도착 15칸 최솟값 **1.006** — 같은 판 응답시작은 **0.950** 까지 내려갔다(판 3 대조군은 한 요청 쌍을 두 시계가 **81ms 차**로 읽는다). 옛 못 `0.95` 였다면 오늘도 **0.4ms 차로 겨우** 통과했을 값이고 반복 616 에서 실제로 죽은 값은 0.948 이다. ②`crawl_delay_e2e` 3판 rc=0(`Crawl-delay:2` 2.019~2.136 · 하한 도메인 1.044~1.119). ③음성 대조 — 못을 `0.5` 로 내린 변이는 e2e 가 **그냥 통과**하고 단위 가드가 **죽는다**(노린 그림). **간격 검사 «호출»을 지운 변이는 e2e 도 가드도 안 죽는다** — 가드가 함수의 «몸»만 읽고 «불리는지»는 안 보기 때문이다. 제품 하한은 `tests/test_frontier.py:319` 가 따로 물어 크롤러가 빨라지지는 않으므로 RED 가 아니라 **발견 후보**로 남긴다(「소스를 읽는 가드는 호출 자리도 세야 한다」). 품질 축은 **크롤 간격·의존성 0 만 통과**, 나머지 다섯은 `src/` 0줄이라 **해당 없음 — 미검증**(계획 104 선례대로 「통과」라 안 쓴다). 남은 여유는 **2~6ms** 로 좁지만 자가 제 잠을 안 섞으므로 앞으로 걸리면 지터가 아니라 신고다. 다음은 **마감** — 계획서 아카이브 → `index.md` 완료 → 병합. 이 계획이 이 밤의 **세 번째**라 마감 뒤 3계획 상한에 닿아 정지한다.
---
