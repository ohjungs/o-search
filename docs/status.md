...
- **사람이 병합했다 — `origin/main` 이 `d1fe3e9` → `bff2580` 으로 움직였다.**
  푸시 뒤 `git ls-remote origin main` 으로 확인한 실측이다(`bff2580 병합:
  loop/passage-cost-band — 계획 57~65 (반복 322~380)`). 계획 57~65 중 **반복 380 까지**가
  `main` 에 들어갔고, 오늘 e2e 커밋(`f5c3849`)은 아직 `loop/passage-cost-band` 에만 있다.
  루프는 `main` 을 **여전히 안 건드린다**(직접 커밋 0 · PR #7 무접촉 · `gh` 호출 0) —
  기록만 정정한 것이고, 반복 344 이후 문서가 되풀이하던 `d1fe3e9` 는 이제 낡았다.
  **다음 계획 phase 는 기점을 `main` 에서 다시 재야 한다** — 계획 58 이 「기점은 `main` 이
  아니라 아카이브 커밋」으로 판단했던 근거(고칠 단언이 `main` 에 0건)가 병합으로 뒤집혔다.
