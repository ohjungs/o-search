...
| plan_graceful-interrupt | 진행 | loop/graceful-interrupt (미개설) | 0/4 | — | **34.** Ctrl-C 가 최대 **69.57초** 안 먹는 것(`digest.md` `[4]`). 계획 탐침이 그 90초 추정을 **70초로 정정**하고 분해했다(소켓 30 + 재시도 잠 40). **설계 있음** — `stop` 이벤트 하나로 **깨우고 접는다**(B, 정공법). **네 자리 중 둘은 0줄**: 소켓은 재시도를 접으면 10초로 줄고, `futures.wait` 는 워커 완료가 깨운다(탐침 실측 0.51s/3.01s — 계획 탐침의 20.01초는 *워커가 계속 도는* 경우였다). 목표 SIGINT 후 **12초 안 종료**·유실 0·중단 중에도 `Crawl-delay` 준수 |
