...
note: **계획 105 `crawl-gap-ab` 를 세웠다(밤의 셋째, 상한이다).** 출처는 `digest.md ## 다음 계획 후보` `[6]` — 「`deadline_e2e.py` 가 1초 하한에서 간헐로 갈린다」. **오늘 재현했다**: 2판 중 1판이 `대조군: 포트 52622 간격 0.948초` 로 죽었고, 같은 날 `crawl_delay_e2e.py` 는 3판 전부 통과(최소 1.06·1.12·1.15). **갈리는 조건은 동시성이다** — 앞은 4도메인 동시, 뒤는 2도메인 순차. **후보가 걸어 둔 여는 조건이 채워졌다**: 「1초를 지켰나」인지 「±50ms 로 지켰나」인지는 `docs/specs/concept.md:25` 가 이미 정했다 — 「1초 이상은 기능이 아니라 전제 조건」. 그러면 지금 못 `0.95` 는 **사양보다 느슨하고** 0.948 은 지터가 아니라 신고다. **모르는 것은 깨지는 쪽이 발신인지 수신인지**다 — 읽어서는 안 보인다(`frontier.mark_sent` 는 `max` 라 늦은 쪽으로만 가고 `crawl.py:382` 가 워커의 실제 발신 시각을 건다). 그래서 스텝 1 은 **둘을 같은 판에서 재는 탐침**이고 스텝 2 가 판정대로 고친다. 어느 쪽이든 못은 사양대로 **1.0 으로 올라간다** — 낮추는 것이 아니라 올리는 일이다. **탐색 여덟 출처 실측**: 1·2 순위 0건(전수 809 OK · 린터 없음) · 3순위 0건(코드 `TODO` 는 `tests/test_indexer.py:1027` 의 픽스처 문자열 하나뿐) · 4순위 0건(`docs/candidates.md` 가 **없다** — 후보 `[6]` 이 진단한 구조적 0건 그대로) · 8순위 이슈 0건. 중복 방지 5곳 통과
---
