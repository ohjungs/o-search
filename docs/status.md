...
note: **빈손 두 밤을 컨셉으로 끊었다.** 여덟 출처는 이번에도 전부 0건이지만(전수 788 OK · `TODO` 0 · 이슈 0 · `candidates.md` 없음 · 보류는 보안 건 하나) 계획 77 이 쓴 길을 다시 썼다 — `docs/specs/concept.md` 를 자로 댄다. **경량 2**(검색 서버 상주 메모리 2GB · 판정: RSS 측정)가 **판정 방법까지 적힌 채 한 번도 안 돌아간 축**이다(RSS 재는 코드 **0줄**). 계획 102 `serve-rss` 착수. **설계 확정(반복 595)** — 갈래 셋 중 C(진짜 진입점을 자식으로 띄우고 밖에서 `ps`). 탐침이 덤으로 답을 줬다: **색인 8배에 띄운 직후 RSS 가 바이트 단위로 같다**(17,008 KB) — `serve.py:222` 가 요청마다 DB 를 연다
---
...
산출물이 측정 도구라 별도 시나리오를 안 만들었다(계획서). 대신 **이 자가 제 존재 이유대로
작동하는지**를 밟았다 — 설계 4절이 「누군가 캐시를 얹는 날 평평하지 않게 된다」고 적었으니
e2e 의 질문은 **그날이 오면 정말 그러나** 하나다.

- **시나리오 1(정상 트리)**: 색인 8배에 신호 −64.0 KB · 바닥 176.0 KB → 점추정 없음 ·
  상한 66.7 MB 이하(예산의 3.3%). **rc=0**
- **시나리오 2(회귀를 심었다 · 되돌린 트리)**: `serve.main` 이 뜰 때 색인을 통째로 올리게
  세 줄을 심으니 **0.8274 KB/문서 · 100만 외삽 825.6 MB = 예산의 40.3%**. 못 재던 값이
  회귀 하나에 40배가 된다 — **이 자는 제 일을 한다.** `src/` 는 되돌렸다(`git status` 무변).
- **[E102-1] 그 판이 거짓 음성을 열어 보였다(고쳤다 · 같은 파일 6줄).** 회귀가 **기동 때**
  올리면 「뜬 직후」끼리도 색인을 따라 갈려 **바닥이 신호와 같이 커진다**(2,848 vs 2,688 KB
  — 여유 160KB). 리뷰 [R102-3] 이 넣은 「신호가 바닥 아래면 평평」 규칙이 **바로 그 회귀를
  평평으로 보고할 뻔했다.** 바닥에 천장을 뒀다: `NOISE_CEILING_KB = 512`(실측 잡음
  32~176 KB · 회귀 2,816 KB). 그 위는 잡음이 아니라 신호로 읽고 기울기를 **뜬 직후·질의 뒤
  중 큰 쪽**으로 낸다. 두 갈래 재실행으로 확인.
- **리뷰가 넣은 가드가 리뷰 다음 판에 바로 반례를 맞았다.** 가드를 세운 판과 그 가드를
  실물 회귀에 대 보는 판은 다른 일이라는 표본이다 — 대 보지 않았으면 이 자는
  **평평하다고 거짓말하는 자**로 남았다.

전수 **788 OK**(20.7s) · `src/` **0줄** · 결과 `docs/e2e/serve-rss/result.md`.
