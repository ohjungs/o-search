...
note: **계획 98 스텝 1/1 개발 완료 — 전수 763 → 769 OK.** 570 이 갈림길을 판단이 아니라
  측정으로 닫았고(미등재 5개가 **전부 rc=0**) 571 이 명부(13종 → 18종)와 **명부를 무는 자**를
  한 스텝에서 세웠다. **스텝 둘을 하나로 합친 것이 이 반복의 판단이다** — 쪼갠 대로 가면
  ②실패를 본 뒤 ③구현 전에 **빨간 트리를 커밋**하는데, 게이트 ⑨ 가 그 트리를 `main` 에
  옮긴 전례가 있다. **밤이 못 여는 것과 밤이 안 잰 것은 다르다** — 564 와 같은 자리다.
---

# 현재 상태

**활성 계획 98 `e2e-roster` · 스텝 1/1 개발 완료.** `docs/plan_e2e-roster.md` ·
브랜치 `loop/e2e-roster`(기점 `4bed2e1`). 다음 반복은 **테스트 phase** — 새 자의 이빨을
변이로 재고 갭을 찾는다.

**스텝 둘을 하나로 합쳤다** — 처음 쪼갠 대로 가면 `dev.md` 0절 TDD 주기가 두 커밋에 걸쳐
끊기고 **②실패를 본 뒤 ③구현 전에 빨간 트리를 커밋해 남긴다.** 게이트 ⑨ 가 그것이 가설이
아님을 이미 보였다(autocommit 이 스텝 도중 트리를 `main` 에 병합한 `d74ed2f`). 주기는 한
스텝 안에서 닫았다. 계획서 「스텝」 절에 사유를 박았다.

**한 것**: `e2e_roster_gap` + `E2eRosterGapTest`(갈래 4건) + `E2eRosterTest`(실물 2건) ·
`project.md` 13종 → **18종**. **RED 를 눈으로 봤다** — 메시지가 다섯을 정확히 짚었다.
**음성 대조**: `recrawl_e2e` 를 명부에서 지우니 자가 그 이름 하나를 물었고 원복 후 초록.
**이 자는 부분 문자열로 세지 않는다** — `interrupt_e2e` 가 `indexer_interrupt_e2e` 의
부분 문자열이라 실재하는 함정이고, 갈래 테스트가 그 자리를 붙든다.
**기존 자가 내 diff 를 물었다** — `test_readme.py` 의 (단위, e2e) 대조가 763→769 를 잡아
README·`project.md` 양쪽 숫자를 고쳤다. `project.md` 는 이름 줄을 접어 **예산 60줄을 그대로 지켰다.**
...
`pages` **52,172** · `docs` **40,347** · 2.48GB · 전수 **769 OK**(20.4s · 763 → **+6**).
**e2e 는 13종이 아니라 22종이다** — 시나리오 **18** + 측정 **4**. 570 이 미등재 5개를 처음
돌려 **5/5 rc=0** 을 확인했고 571 이 명부와 무는 자를 세웠다.
필수 읽기 **285줄**/600 (`project.md` 60 + `status.md` 112 + `history_current.md` 113) — 여유 315줄.
