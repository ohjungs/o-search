...
class TestIndexPages(unittest.TestCase):
    ...
    def test_update_touches_only_the_row_the_join_handed_it(self):
        ...
        그 뒤 남은 한 행을 두 번째 쌍이 또 지운다 — 결과가 1행으로 접힌다. 그 경로는
        **건네받고도 아직 처리하지 않은 행을 미리 지운 것**이라 우연이다.

        **우연이지만 공짜는 아니었다** — 접는 것이 중복을 지우는 유일한 경로였고,
        `rowid` 로 바꾸면서 그것을 잃었다. 중복은 `index_pages` 동시 실행으로 실제로
        생기고(읽기가 트랜잭션 밖이다 · 실측 10 pages → `docs` 20행) 이제 **영구히
        남아 검색에 같은 URL 이 두 번 나온다**. 이 단언은 그것을 「옳다」고 못박는 것이
        아니라 **열쇠가 무엇인지**를 고정하는 것이고, 중복 자체를 막는 일은 트랜잭션
        경계에 있어 승인 대기다(`status.md ## 사람 결정 대기`).
...
class TestPassages(unittest.TestCase):
    ...
    def test_cap_and_passage_limit_together_stay_inside_the_budget(self):
        ...
        # 올려도 예산이 깨지므로 곱해서 본다. 한도는 사양 성능 5 의 예산 **500ms 정각**
        # 이다 — 옛 배정치 1/3 은 «모양 배수를 흡수하는 자리» 였는데 계수가 이제 캡 최악
        # 모양의 실측이라 흡수할 배수가 없다(계획 57).
        # **계수 옆에 그 숫자를 낸 입력의 모양을 적는다** — 이 계수는 «모양» 이름으로는
        # 재현이 안 돼 세 번 낡았다(0.118 «한글·태그 촘촘» → 0.352 → 0.44 «위키·CMS» ·
        # 계획 48·51). 밀도 축(ms/1000자 ≈ 0.0025 × 태그/1000자)도 한 점 맞춤이라 같은
        # 밀도 250 에서 모양이 1.25배 갈린다. **그래서 계수를 축의 끝 — 캡을 채운 최악
        # 모양(`<p>` 만 반복 · 태그 333개/1000자) — 에서 잡는다.** 오늘 값은 idle
        # min-of-3 20표본이 0.917~0.974 라 **표본 위로 올려 1.00** 이고, 10건이면
        # **350ms · 예산의 70%** 다(계획 57 e2e 재측정 [R57-1] — 앞의 0.95 는 «한가할 때
