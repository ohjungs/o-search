...
note: **스텝 2 끝 — 계획 105 의 개발이 닫혔다. 제품 `src/` 는 0줄이다.** 셋을 같이 했다: ① 단언을 `ARRIVAL_LOG`(도착) 쪽으로 옮겼다 ② 못을 `0.95` → **`1.0`** 으로 올렸다(두 파일) ③ `e2e/deadline_e2e.py` 의 「**예산이** 1초 하한을 깎았다」를 「1초 하한이 깨졌다」로 고쳤다 — 오늘 죽은 것이 **예산 없는 대조군**이었으니 옛 문구는 사실과 달랐다(`[R71-2]` 와 같은 자리). **덤으로 하한 못 하나**: 도착 로그가 안 차면 루프가 한 번도 안 돌아 조용히 통과한다 — `len(gaps) >= len(served)` 로 막았고 변이 M4 가 그것을 실증했다. **사양 인용 가드가 물었다** — `concept.md:25` 를 대면서 큰따옴표 조각을 안 적어 전수가 2건 빨갰다. 「1초 이상」(낫표)이 아니라 `"도메인당 요청 간격 1초 이상"`(큰따옴표)이라야 그 자가 사양 원문과 대조한다. **남은 여유는 2ms 다**(56칸 도착 최솟값 1.002s) — 얇지만 **재는 자리에 더는 내 잠이 안 섞이므로** 다음에 이 못이 물면 그것은 지터가 아니라 신고다. 다음은 테스트 phase.
---
