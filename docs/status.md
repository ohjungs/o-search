...
note: **테스트 스텝 완료 — 변이 8판을 전부 죽였다. 전수 779 OK.**
  갭 하나를 찾아 메웠다(빈 명부 0종은 이 자가 조용하고 **형제가 문다**). 건수 가드가
  또 물어 778 → **779**. 다음 반복은 **리뷰 phase**.
---

# 현재 상태

**계획 99 `e2e-roster-count` 테스트 완료 — 다음은 리뷰 phase.**
...
**변이 8판 전부 잡혔다**(반복 577 · `PYTHONDONTWRITEBYTECODE=1` **과**
`PYTHONPYCACHEPREFIX`). 유령 판정 무력화 · 라벨 판정 무력화 · 맨 식별자 필터 제거 ·
앵커 실종 신고 제거 · `doc` 칸 죽이기 · 유령을 라벨 뒤로 · **이름 추출 전멸** · 형제의
빠진-이름 판정 무력화.

- **`doc` 칸이 이번엔 살아서 죽었다** — 계획 98 의 M9 가 살아남았던 그 축을 실물 1건이 문다.
- **하한 못이 없어도 되는 것을 M7 이 증명했다** — 이름 추출을 전멸시키자 실물 테스트가
  라벨 18 과 0 의 어긋남으로 빨개졌다. 주석의 주장이 이제 측정값이다.
- **구멍은 하나였다 — 빈 명부(0종)는 0 == 0 이라 조용하다.** 여기에 못을 더 박지 않고
  **형제 `e2e_roster_gap`(모집단이 디스크)이 문다**는 것을 테스트로 잰다(M8 이 그 테스트만
  죽인다). 「둘이 서로를 대체하지 않는다」가 주석에서 측정으로 내려왔다.
- **건수 가드가 두 반복 연속 물었다**(778 → **779**). `README.md:104` · `project.md:13` 갱신.
