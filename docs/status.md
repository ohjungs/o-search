...
note: **스텝 1 끝 — 탐침이 갈랐다. ②수신만 깬다.** 10판 원본은 아래 절에 있다. **도착 시각으로 잰 간격의 최솟값은 26개 중 하나도 1.0 밑으로 안 내려갔다(전체 최소 1.007s).** 1.0 을 깬 수는 **전부 응답시작 쪽**이고 최저가 0.907s 다(판 3 대조군). 즉 **크롤러는 1초를 지켰고 재는 자리가 틀렸다** — 핸들러가 `time.sleep(delay)` **뒤**에 시각을 찍는다(`e2e/deadline_e2e.py:95~97`). 도메인당 `delay` 는 상수라 간격에서 상쇄되지만 **`time.sleep` 의 초과분은 매번 달라** 그 차이가 그대로 간격에 더해지고 빠진다. 4도메인 동시일 때 스윙이 **100ms 넘게** 벌어져(1.030 → 0.907) 못 `0.95` 를 뚫는다. `crawl_delay_e2e` 가 2도메인 순차라 안 깨지던 것도 같은 이유다. **후보 `[6]` 은 크롤러 버그가 아니라 자의 버그였다.** 그래서 스텝 2 는 계획서의 ② 갈래로 간다 — 제품 `src/` 는 **안 건드린다**(설계 트리거 재검토도 해당 없음). 단언을 `ARRIVAL_LOG` 쪽으로 옮기고 못을 `1.0` 으로 올리고 `:133` 문구를 원인 중립으로 고친 뒤 변이 2종을 친다.
---
