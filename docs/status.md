...
note: **리뷰 완료 — 보고 1건([R99-1] 중복 단언), 자동 수정으로 닫았다. 전수 779 OK.**
  패스 A 를 **다른 세션에 못 넘겼다**(이 하네스에 Agent 도구가 없다) — 룰대로 두 패스를
  순서대로 혼자 돌았고 그 사실을 적는다. 다음 반복은 **e2e phase**(시나리오 2종).
---

# 현재 상태

**계획 99 `e2e-roster-count` 리뷰 완료 — 다음은 e2e phase.**
...
**변이 9판 전부 잡힘**(577 의 8판 + 리뷰가 더한 M9 「점만 허용하는 느슨한 필터」).

- **[R99-1] 자동 수정** — `test_a_dotted_token_is_not_a_name` 이 「정상」 픽스처와 **같은
  호출**이었다. 축이 하나 늘어난 척만 하고 재는 것은 같다. 라벨을 1 로 조여 다시 세웠고,
  새 변이 M9 가 그 갈래를 죽이는 것을 확인했다. 테스트 수는 그대로라 README 는 안 건드렸다.
- **패스 A 를 다른 세션에 못 넘겼다** — 이 세션에 Agent 도구가 없다. `review.md` 0절의
  「넘길 수단이 없으면 순서대로 두 패스」를 따랐고, **구조가 아니라 규율로 돈 리뷰**임을
  여기 남긴다(같은 사람이 쓰고 본 diff다).
- **구멍은 여전히 하나 — 빈 명부(0종)는 0 == 0 이라 조용하다.** 못을 더 박지 않고
  형제 `e2e_roster_gap` 이 문다는 것을 테스트로 잰다(M8 이 그 테스트만 죽인다).
- 건수 가드가 두 반복 연속 물었다(770 → 778 → **779**). `README.md:104` · `project.md:13`.
