...
`index.md` 에 계획 57 행을 **진행 0/1** 로 넣고 원격 줄을 「열린 PR 1건(#11)」로 고쳤다 —
안 넣으면 다음 반복의 중복 방지가 이 계획을 못 본다.
...
러너를 다섯 번 불렀고 **판정 줄은 넷**이다(가드 2 · 전수 2 · 하나는 `-s tests -t .` 로 인자를
틀려 `ImportError`(rc 1)로 튀어 0개 돌았고 `project.md` 의 `discover -b tests` 로 고쳤다).
