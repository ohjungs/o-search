...
note: **계획 98 이 `main` 에 들어갔다 — 게이트 ⑪ 을 강제 푸시 없이 닫았다**(`b8f0bb1` ·
  전수 770 OK). 이어서 **계획 99 `e2e-roster-count` 착수**(탐색 8순위 전수 · 6순위에서 나왔다).
  다음 반복은 **개발 스텝 1/1**.
---

# 현재 상태

**계획 99 `e2e-roster-count` 계획 phase 완료 — 다음은 개발 1/1.**
계획서 `docs/plan_e2e-roster-count.md` · 브랜치 `loop/e2e-roster-count`(기점 `b8f0bb1`).

**게이트 ⑪ 이 닫혔다 — 사람이 고를 두 갈래 밖에 셋째 길이 있었다.** 원격의 자동 스냅샷
`55a57db` 와 내 리뷰 커밋 `e31d115` 는 **트리가 바이트까지 같다**(`git diff` 빈손). 그래서
강제 푸시(㉮)도 되감기(㉯)도 필요 없고 **그냥 병합하면 된다** — 충돌 셋은 전부 내 쪽이
상위집합이라 `--ours` 로 풀었고, 병합 결과 트리가 `fb1ac33` 과 **0줄 차이**임을 확인한 뒤
커밋했다(`a9a14eb`). `merge-to-main.sh` 가 전수 **770 OK** 위에서 `main` 을 밀었다(`b8f0bb1`).
**일반화: 「강제로 밀까 되돌릴까」로 갈림길이 둘로 보이면 두 갈래가 같은 가정(히스토리를
고쳐야 한다)을 공유하는지 본다** — 여기서는 고칠 것이 없었고 합치면 됐다.

**탐색(575)**: 1·2·3·4·8순위 **0건**(전수 770 OK 18.7s · 린터 없음 · 코드 `TODO` 0 —
`test_indexer.py:1027` 의 HTML 픽스처 문자열은 알려진 오탐 · `candidates.md` 없음 · 이슈 0건).
**5순위 0건** — 보류는 `userinfo-leak` 하나고 **보안 건이라 밤이 영구히 못 연다**(게이트 ③).
**7순위 0건** — `## 반복 실패` 세 항목의 처방이 전부 이미 섰다(`archive_gap`·`iter_gap`·리뷰 예산).
**6순위에서 하나**: `[6]` 「18종 라벨이 명부 길이와 안 묶였고 «이름 줄은 기계가 읽는 줄»을
재는 자가 0개다」. **여는 조건을 다시 판정했다** — 적혀 있던 조건(「e2e 를 더하거나 지우는 날」)은
**라벨 축**의 것이고 **유령 축은 574 에 이미 발동했다**(`E2eRosterTest` · `rc=2`).

**계획서를 쓰기 전에 실물을 쟀고, 등재된 처방이 틀린 것을 찾았다.** digest 는 처방을
「뽑은 이름 수 == 라벨 == `glob` 개수」로 적었는데 실측은 **18 == 18 ≠ 22** 다 — `glob` 은
측정 넷을 포함하고 그 넷은 `측정` 항목이 부른다. 닫을 등식은 **둘**(수 == 라벨 · 각 이름이
디스크에 실재)이고 계약 3 에 적었다. `## 반복 실패` 의 「등재된 처방은 근거지 설계가 아니다」를
그대로 밟은 자리다.
...
결과를 낸다. 그래서 전수는 밤마다 돌린다(770 OK · 18.7s · Python 3.9.6).
