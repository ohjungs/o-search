...
## 원격이 움직였다 — 계획 54 문서의 전제 하나가 낡았다

**사용자가 PR #7·#8 을 둘 다 병합했다**(2026-09-04 11:23 UTC · `gh pr list` 로 확인).
`git ls-remote origin HEAD` 가 `687a159`(계획 47)가 아니라 **`7db3acb`**(«Merge pull
request #8 from ohjungs/loop/passage-db-state»)다. 즉 **계획 48~53 이 이제 `main` 에 있고**,
이 브랜치의 기점 `91731e4` 도 그 안에 들어갔다. 계획서 기점 절과 `index.md` 의
«PR #7 은 OPEN·미병합» 은 **기록 시점의 사실**이라 안 고쳤다(append 원칙). 다만
**다음 phase 는 그 문장을 오늘의 사실로 읽으면 안 된다** — 계획 55 의 기점 판단과
계획 54 의 병합 경로가 이 사실 위에서 갈린다. `main` 직접 커밋은 여전히 하지 않는다.
