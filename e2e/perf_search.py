...
# 넓게 걸리는 질의·좁은 질의·무결과·깊은 페이지를 섞는다. OFFSET 은 선형으로 자라므로
# 깊은 페이지가 가장 느린 쪽이고, 기준선은 가장 느린 쪽을 포함해야 쓸모가 있다.
QUERIES = [
    "/search?q=%s",                    # 전 문서에 걸린다
    "/search?q=%s&page=100",           # 상한 페이지 — OFFSET 990
    "/search?q=" + urllib.parse.quote("문서 0777"),   # 한 문서만
...
def build_index(db_path, docs):
    ...
    db.executemany("INSERT INTO pages VALUES (?, ?, 200)", [
        ("http://p.test/%05d" % i,
         "<html><title>문서 %04d</title><body><p>김치 %s "
...
def main(argv):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            paths = [p % quoted if "%s" in p else p for p in QUERIES]
