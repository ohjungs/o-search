...
note: **스텝 1 닫혔다 — 재는 자가 섰고 빨갛다**(`tests/test_verdict_wrapper.py` 6건 중 **5건 실패**, 전수 818건 `FAILED (failures=5)` `rc=1`). 실패 메시지가 「없는 파일」이 아니라 **「마지막 줄이 판정을 안 실었다」**를 가리킨다 — 완료 기준 그대로다. 여섯째(`test_the_fake_runner_actually_makes_the_problem`)만 **지금 초록이어야 맞다**: 래퍼 **없이** 같은 파이프를 태우면 판정이 실제로 사라진다는 대조군이라, 이게 빨가면 나머지 다섯은 증거가 아니다. **그리고 이 반복이 서른일곱 번째 표본을 냈다** — 전수를 친다면서 `discover -b`(시작 디렉터리 **누락**)를 쳐서 **`Ran 0 tests / OK / rc=0`** 을 받았다. 파이프를 안 붙였는데도 **0 을 세면서 초록**이었다 — 같은 가족의 **새 얼굴**이고, 조항(「맨몸으로 돈다」)이 못 덮는 자리다. 잡아낸 것은 눈이 아니라 **`project.md` 에 적힌 명령과 대 본 것**이다. 후보로 등재했다. **다음은 스텝 2 — `scripts/verdict.sh` + 변이 3판.**
---
