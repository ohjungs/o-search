...
def main(argv):
    ...
    #
    # **천장은 «도메인 수» 로만 센다 — `Crawl-delay` 가 긴 도메인이 있으면 실제보다
    # 높게 잡힌다.** 12도메인 중 `www.gnu.org` 하나가 4초를 요구하면 참값은 11.25 인데
    # 여기서는 12.00 으로 나온다(2026-09-09 실물). 정확히 재려면 `Frontier` 의 도메인별
    # 간격 표가 필요한데 CLI 는 그것을 안 들고 있고, 이 줄의 목적은 **자릿수 안내**지
    # 계량이 아니다. 틀리는 방향도 안전한 쪽이다 — 천장을 높게 부르면 「아직 여유가
    # 있다」로 읽혀 간격을 깎을 이유가 안 생긴다. 실측이 이 값을 **넘으면** 그때는
    # 짧은 크롤이거나 이 근사가 깨진 것이라, 둘 다 요약을 다시 볼 신호다.
