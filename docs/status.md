...
note: **리뷰 — 첫 소견이 내 것이다. [R104-1] 612 의 커밋 메시지와 이 칸이 「전수 809 OK · 예산 574/600」이라 적었는데 그 수는 기록을 붙이기 전에 잰 것**이다. 붙이고 나니 **603 > 600** 이라 자가 물었고, `&&` 사슬이 `tail` 뒤에 있어 실패해도 커밋이 나갔다 — 맨몸 규율이 경고하는 바로 그 꼴을 **내가 검증 명령에서 냈다**. **가드가 제 첫 밤에 제 목적의 회귀를, 그것도 루프 자신의 기록에서 잡았다**(계획 97 의 예측 그대로). 처방은 상한을 올리는 것이 아니라 **내 기록을 규격에 맞추는 것** — SKILL.md 의 append 규격은 **5줄**인데 609~612 가 11·10·10·9 였다. 넷을 접어 **603 → 593**. **[R104-2]** 계획서 완료 기준의 변이 「상한을 601 로 올리면 초록」이 틀렸다 — 그날 합계가 603 이라 601 로도 빨갛다. 실제로 돌린 것은 700 이고 계획서를 정정했다. **주석 대조는 통과**: 「파일별 상한 합 490 < 600」·「`HistoryCapTest` 가 무는 유일한 live 파일 상한」·「`LivePlanSyncTest` 가 `PLAN_SLUG` 를 이미 문다」(`live_gap:19` 실측) 셋 다 사실이다. 남은 것은 e2e
---
