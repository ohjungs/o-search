...
| rc | 뜻 | 호출자가 할 일 |
|---|---|---|
| **0** | 성공 (결과 0건도 성공이다) | 다음 단계로 |
| **1** | **환경이 안 됐다** — DB 없음·pages 없음·옛 색인·DB 손상·락·포트 점유 | 환경을 고친다. 안 잡힌 예외도 같은 값이다 |
| **2** | **명령줄이 틀렸다** — usage·플래그 값·가져올 수 없는 시드 | 사람이 명령을 고친다 |
...
```bash
PYTHONPATH=src python3 -m unittest discover -s tests   # 단위 462건
