...
- **러너가 아닌 곳에 파이프 1회는 적어 둔다** — `git push … 2>&1 | tail -3`. 판정
  대상이 아니라 진행 로그였고, 원격 SHA 는 그 뒤 **맨몸 `git ls-remote`** 로 따로
  읽었다(`872dfca23b31…`). 규율의 글자는 러너를 겨누지만 방아쇠는 같은 것이라 센다.

## 원격을 다시 읽은 값이다

- 브랜치 **`loop/focus-ring-combinator`**(기점 `20ee8d5`). 테스트 커밋 뒤 푸시하고
  **원격을 다시 읽었다**: `HEAD` **`872dfca23b31…`** = `ls-remote` **`872dfca23b31…`**.
