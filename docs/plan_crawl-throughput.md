...
- ~~**robots.txt 왕복이 도메인 간격 시계에 안 실리는 것** (digest `[4]`)~~
  **→ 절반은 범위 안으로 들어왔다 (2026-08-27, 스텝 1이 실측으로 잡았다).**
  간격 시계가 팝 시각에서 시작해 **원본 코드가 이미 0.819초 간격을 내고 있다.**
  동시화는 이걸 반드시 악화시키므로(간격을 1초까지 좁히는 것이 목적이라
  모든 도메인의 첫 간격이 0.6초가 된다) 스텝 2가 **발신 시각 기록**으로 고친다
  → 설계 계약 9. **robots.txt 요청 자체를 간격 시계에 싣는 것은 여전히 범위 밖**
