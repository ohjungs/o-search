...
note: **이 밤은 여기서 끝난다 — 정지 사유 「탐색 막힘(근거 없음)」.** 반복 586 이 짧은 경로
  `test-file-leak` 를 열고 닫았고(`main` `6a1af58` 병합분), 반복 587 의 재탐색은 여덟 출처가
  **전부 0건**이다. 등재된 여는 조건 셋을 실물에 대 봤는데 셋 다 아직 닫혀 있다 —
  e2e 도우미 0건(22/22 진입점) · `fetcher` 는 여전히 non-2xx 본문을 `None` 으로 버린다
  (`fetcher.py:52`) · 제품 CSS 에 괄호 셀렉터 0곳(정적 CSS 파일 자체가 없다).
  `discover.md` 2절대로 **상상해서 계획을 만들지 않고** 멈춘다.
---
...
- 이미 한 것: **끝났다 · `main` 에 들어갔다**(`6a1af58` · 전수 784 OK). 빨강(12) → 고침 →
  초록(0) · `index.md` 등재 완료 · digest `[4]` 의 여는 조건을 「열렸고 같은 반복이 닫았다」로 갱신.
...
**반복 586 짧은 경로 `test-file-leak` 은 `main` 에 들어갔다**(`6a1af58` · 전수 784 OK).
위 「짧은 경로」 절이 근거와 완료 기준이고, 등재는 `index.md` 마지막 행에 있다.

**반복 587 재탐색은 빈손이다 — 정지 사유 「탐색 막힘(근거 없음)」.** 여덟 출처 0건:
전수 784 OK · 린트·타입체커 없음(`project.md:13`) · 코드 `TODO` 0(유일한 문자열은
`test_indexer.py:1027` 의 픽스처 HTML 안이다) · `candidates.md` 없음 · 보류는 보안 건
하나(밤이 영구히 못 연다) · 6순위 후보 절은 주석만 남아 비었다 · 8점 미만 후보들은
여는 조건 미충족 · 이슈 0. **여는 조건 셋을 실물에 대 본 것이 이번 탐색의 값이다** —
셋 다 아직 닫혀 있음을 확인했고, 그것이 「안 열었다」와 「안 봤다」를 가른다.
