...
**30 `readme-perf-audit` · 31 `port-zero-cover` DONE**(둘 다 짧은 경로).
열린 계획 없음 · 보류 패치 0건. 단위 **430건 OK** · e2e **17종 전부 rc=0**.
브랜치 `loop/readme-perf-audit`(기점 `main` `4d8adac`) — 병합은 사용자 판단이다.

**설계는 둘 다 건너뛰었다** — 새 모듈 없음 · 파일 각각 2개·1개 · 데이터 구조 무관 ·
되돌리기 쉬움 · 보안 무관. 31 은 `src` 를 **0줄** 고쳤다(없던 것은 증거뿐이었다).
...
## 반복 149 — 31 `port-zero-cover` (짧은 경로)

- 무엇: `serve --port 0`(임의 포트)에 단위 커버가 0 이던 것을 닫았다. 근거: `digest [4]`.
- 왜: `0` 의 뜻이 플래그마다 다른데(`--max` 는 거절, `--port` 는 "아무 포트나")
  파서는 범위를 안 보므로 **그 비대칭을 지키는 자리가 `serve.main` 한 줄뿐**이었다.
  상한을 조이다 `port < 1` 을 같이 넣으면 단위 429건이 전부 초록이고 e2e 넷만 죽는다.
- 완료 기준: 그 변이가 단위에서 죽는다.
- 이미 한 것: 전부. `src` **0줄** · 기존 `TestCliArgs` 에 65535 테스트의 짝 하나.

**변이 3종**: ① 하한 `< 1` 추가 → **새 테스트 하나만** 죽음 ·
② `number_flag(...) or 8000`(0 을 기본값으로 삼킴) → **새 테스트 하나만** 죽음 ·
③ 상한 제거 → 기존 둘이 죽음(상한은 이미 덮여 있었다). ①②가 429건 중 그 한 건만
죽인 것이 "e2e 넷이 유일한 파수꾼" 이라는 진단이 **사실이었다**는 증거다.

**30 이 붙인 이빨이 첫 실사용에서 울었다** — 테스트를 하나 더하자 README 의
`단위 429건` 이 그 자리에서 FAILED 를 냈다. 이제 사람이 세어 고칠 일이 아니다.
